...
const AddTaskCard = ({ isOpenModel, setIsOpenModel }) => {
  ...
  const addtaskschema = z.object({
    ...
    startdate: z.coerce.date().min(today, "Start date must be today or later"),
    enddate: z.coerce.date().min(today, "End date must be today or later"),
    // priorty: z
    //   .string()
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        <label className="text-gray-500 text-xs mb-1">Attachments (Links)</label>
      ...
      {/* <div className="flex flex-col">
        <label className="text-gray-500 text-xs mb-1" htmlFor="priorty">
          Priority
        </label>
        <select
          id="priorty"
        ...
        {errors.priorty && (
          <p className="text-red-600 text-xs">{errors.priorty.message}</p>
        )}
      </div> */}
