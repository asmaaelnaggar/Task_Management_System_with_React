...
const AddTaskCard = ({ isOpenModel, setIsOpenModel }) => {
  ...
  const addtaskschema = z.object({
    ...
    startdate: z.coerce
      .date()
      .min(new Date(), "Start date must be now or later").transform((date) => date.toISOString()),
    enddate: z.coerce.date().min(new Date(), "End date must be now or later").transform((date) => date.toISOString()),
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        <label className="text-gray-600 dark:text-gray-300 text-xs mb-1">
          Start Date
        </label>
        <input
          type="date"
