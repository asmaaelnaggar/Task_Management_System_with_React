...
export default function AccountComponent() {
    ...
    const handleDelete = () => {
        if (
            ...
            localStorage.removeItem("currentUser"); // Assuming "user" is the key for user data in local storage
            alert("Account deleted successfully!");
            navigate("/"); // Redirect to home or logout page
        }
    };

    return (
        <div className={theme === "light" ? "bg-[#F3F4F8] flex-1 p-8" : "bg-gray-900 flex-1 p-8"}>
            <h2 className={theme === "light" ? "text-black text-2xl font-bold mb-6" : "text-white text-2xl font-bold mb-6"}>Settings</h2>
            <div className={theme === "light" ? "bg-[#F3F4F8] p-6 rounded-2xl shadow" : "bg-gray-900 p-6 rounded-2xl shadow"}>
                <div className="mb-6">
